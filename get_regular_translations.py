    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            with output_file_name.open('a') as f:
                json_line = json.dumps(df.loc[index].to_dict())
                f.write(f'{json_line}\n')

        except Exception as e:
            print(f"Error processing index {index}: {e}")

async def get_regular_translations( 
        llm_service
) -> str:
    ...
    output_directory = Path('./Data/Output/translations')
    await get_translations(
        df, 
        llm_service, 
        output_directory
    )
    return './' + str(output_directory / f'big_c_conversations_test_{llm_service.__name__}.jsonl')
...
if __name__ == "__main__":
    get_regular_translations(get_o1_mini_response)
